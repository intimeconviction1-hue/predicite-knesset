...
export default function AdminResultats() {
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 py-10">
      <h1 className="text-2xl font-black mb-1" style={{ fontFamily: 'var(--font-display)', color: 'var(--p-text)' }}>
        Résultats — saisie manuelle
      </h1>
      <p className="text-sm mb-8" style={{ color: 'var(--p-text-40)' }}>
      ...
      <div
        className="flex items-center justify-between mt-5 rounded-lg border px-4 py-3"
        style={{
          background: 'var(--p-card)',
          borderColor: ecart === 0 ? 'var(--p-gold)' : 'var(--p-border)',
        }}
      >
        <span className="text-sm font-semibold" style={{ color: 'var(--p-text-60)' }}>Total</span>
        <span
          className="text-lg font-black tabular-nums"
          style={{ color: ecart === 0 ? 'var(--p-gold)' : 'var(--p-text)' }}
        >
          {total} / {TOTAL_SIEGES}
          {ecart !== 0 && (
            <span className="text-xs font-semibold ml-2" style={{ color: 'var(--p-text-40)' }}>
              ({ecart > 0 ? `+${ecart}` : ecart})
            </span>
          )}
        </span>
